<template>
	<div v-if="(data && definition && !this.deleted)" class="card mb-1">
		<div class="card-header card-header-primary">
			{{definition[this.data.type_guid].name}}
		...
		<div class="card-block" v-if="expanded">
			<field v-for="field in definition[this.data.type_guid].fields" :data="data.fields[field.name]" :definition="field" v-on:update="update"></field>
		</div>
	</div>
</template>
