...
class UpdateContentTest extends APICommandTestCase
{
	...
	{
		return json_decode(
			file_get_contents(base_path('tests/Support/Fixtures/api_requests/v1/update_content.json')),
			true
		);
	}
	...
	public function validation_ifRegionHasNoDefinition_fails()
	{
		...
		// dd($invalid_data);
		$invalid_data['blocks']['invalid-region-v1'] = $invalid_data['blocks']['test-region-v1'];
	...
	public function validation_ifSectionHasTooManyBlocksValidation_fails()
	{
		...
		$invalid_data['blocks']['test-region-v1'][0]['blocks'][] =
			$invalid_data['blocks']['test-region-v1'][0]['blocks'][1];
		$invalid_data['blocks']['test-region-v1'][0]['blocks'][] =
			$invalid_data['blocks']['test-region-v1'][0]['blocks'][1];
		$invalid_data['blocks']['test-region-v1'][0]['blocks'][] =
			$invalid_data['blocks']['test-region-v1'][0]['blocks'][1];
	...
	public function validation_ifSectionsInRegionAreInWrongOrderValidation_fails()
	{
		...
		$invalid_data['blocks']['test-region-with-multiple-sections-v1'][] =
			$invalid_data['blocks']['test-region-with-multiple-sections-v1'][0];
